...
namespace Log
{
	...
	struct Logger::SubLogger
	{
		...
		auto BornNewWith(NextInfo info)
		{
			if constexpr (not is_same_v<void, decltype(CurryedAccessLog(forward<NextInfo>(info)))>)
			{
				...
				{
					auto newBasicInfo = tuple_cat(BasicInfo, tuple(remove_reference_t<NextInfo>(info)));
				...
				else
				{
					return SubLogger<BasicInfoLimitCount, decltype(newerAccessLog), CurrentBasicInfoTuple>{
						move(newerAccessLog), parent, move(BasicInfo) };
				}
			}
			else
			{
				...
				{
					auto newBasicInfo = tuple_cat(BasicInfo, tuple(remove_reference_t<NextInfo>(info)));
					return NonAccessLogSubLogger{ move(newBasicInfo) };
				}
				else
				{
					return NonAccessLogSubLogger{ move(BasicInfo) };
				}
			}
		}
	...
	struct Logger::NonAccessLogSubLogger
	{
		...
		NonAccessLogSubLogger(CurrentBasicInfoTuple infos) : BasicInfo(move(infos))
		{ }
